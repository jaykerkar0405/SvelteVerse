<script lang="ts">
	...
	async function initializeRoom() {
		if (!browser || !client) {
			error = 'AV room cannot be initialized in server-side rendering';
			isJoining = false;
			return;
		}
	...
	async function endCall() {
		await cleanup();
		goto('/webrtc/av/peer-to-peer?reset=' + Date.now());
	}
	...
	<div class="fade relative flex h-[89vh] w-full flex-col">
		...
		>
			<div class="flex w-full flex-col gap-2 sm:hidden">
				<div class="flex items-center justify-between">
					<div class="flex items-center gap-2">
						...
						<span class="text-sm font-bold tracking-tight">P2P AV</span>
					</div>
					<Button
						size="icon"
						variant="ghost"
			...
			<div class="hidden min-w-0 items-center gap-4 sm:flex">
				...
				<span class="hidden text-lg font-bold tracking-tight lg:block">Peer To Peer AV Room</span>
				<Badge
				...
				>
					...
					{#if isMuted}
						<div
							...
							class="absolute right-4 top-4 flex h-8 w-8 items-center justify-center rounded-full bg-destructive/90 backdrop-blur-sm"
						>
